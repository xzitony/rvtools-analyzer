...
| **VCF 9 Sizing** | Sizes a VCF 9 fleet on the existing hosts. It picks the best cluster to peel hosts off for a dedicated (greenfield) management domain, and you can choose a different one. It checks that the rest of that cluster can still carry its VMs, and flags "not enough capacity for a greenfield management domain build" when it can't. It also sizes a consolidated option where management and workloads share one cluster. The other clusters become workload domains, one per vCenter, per cluster or one for all, and their vCenters and NSX Managers are added to the management domain. You can merge chosen clusters into one workload cluster to see how many failover hosts that saves. Appliance sizes and the storage calculation follow the VCF 9.1.1 Planning and Preparation Workbook. Host fit uses each host's cores and memory, with host failures held in reserve. |
