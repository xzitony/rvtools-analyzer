...
public enum CSVExport {
    ...
    /// Storage in the chosen units (column headed with `gbUnit`), and memory in GiB.
    static func gb(_ mib: Double) -> String { Fmt.storageGB(mib: mib) }
    static func gib(_ mib: Double) -> String { Fmt.memoryGiB(mib: mib) }
    static var gbUnit: String { Fmt.storageGBLabel }
    ...
    public static func vms(_ r: Report) -> String {
        build(["VM", "Power", "Template", "vCenter", "Datacenter", "Cluster", "Host", "Folder", "Guest OS", "OS family", "OS end of support",
               "vCPU", "Sockets", "Cores/socket", "Memory GiB", "Provisioned \(gbUnit)", "In use \(gbUnit)", "Guest capacity \(gbUnit)", "Guest used \(gbUnit)",
               "Disks", "Datastores", "NICs", "Networks", "IPs", "HW version", "Firmware", "Tools", "Tools version",
               "Snapshots", "Snapshot \(gbUnit)", "Oldest snapshot days", "Created", "Issues", "Annotation"],
              r.inventory.vms.map { v in
                  [v.name, v.powerState.rawValue, v.isTemplate ? "Yes" : "No", v.vcenter, v.datacenter, v.cluster, v.host, v.folder,
                   ...
                   gib(v.memoryMiB), gb(v.provisionedMiB), gb(v.inUseMiB), gb(v.guestCapacityMiB), gb(v.guestConsumedMiB),
    ...
    public static func selection(_ vms: [VM]) -> String {
        build(["VM", "vCenter", "Cluster", "Host", "Power", "Guest OS", "vCPU", "Memory GiB", "Provisioned \(gbUnit)", "In use \(gbUnit)"],
              vms.sorted { $0.name.localizedStandardCompare($1.name) == .orderedAscending }.map { v in
                  [v.name, v.vcenter, v.cluster, v.host, v.powerLabel, v.os.name, "\(v.cpus)", gib(v.memoryMiB), gb(v.provisionedMiB), gb(v.inUseMiB)]
              })
    }

    public static func hosts(_ r: Report) -> String {
        build(["Host", "vCenter", "Datacenter", "Cluster", "Vendor", "Model", "CPU model", "Sockets", "Cores", "Threads", "Memory GiB",
               "CPU usage %", "Memory usage %", "VMs", "VMs on", "vCPU (on)", "vCPU:core", "vRAM on GiB", "ESXi", "Build", "Maintenance",
              ...
              r.inventory.hosts.map { h in
                  [h.name, h.vcenter, h.datacenter, h.cluster, h.vendor, h.model, h.cpuModel, "\(h.sockets)", "\(h.cores)", "\(h.threads)",
                   gib(h.memoryMiB), Fmt.num(h.cpuUsagePct, 0), Fmt.num(h.memUsagePct, 0), "\(h.vmCount)", "\(h.vmsOn)", "\(h.vcpuOn)",
                   Fmt.num(h.vcpuPerCore, 2), gib(h.vramOnMiB), h.esxVersion, h.esxBuild, h.maintenance ? "Yes" : "No",
    ...
    public static func clusters(_ r: Report) -> String {
        build(["Cluster", "vCenter", "Datacenter", "Hosts", "Cores", "Memory GiB", "VMs", "VMs on", "vCPU on", "vCPU:core", "vRAM on GiB",
               "CPU usage %", "Memory usage %", "Memory % after losing largest host", "HA", "DRS", "Admission control", "Provisioned \(gbUnit)", "Issues"],
              r.inventory.clusters.map { c in
                  [c.name, c.vcenter, c.datacenter, "\(c.hostCount)", "\(c.cores)", gib(c.memoryMiB), "\(c.vmCount)", "\(c.vmsOn)", "\(c.vcpuOn)",
                   Fmt.num(c.vcpuPerCore, 2), gib(c.vramOnMiB), Fmt.num(c.cpuUsagePct, 0), Fmt.num(c.memUsagePct, 0),
    ...
    public static func datastores(_ r: Report) -> String {
        build(["Datastore", "vCenter", "Type", "Capacity \(gbUnit)", "Used \(gbUnit)", "Free \(gbUnit)", "Free %", "Provisioned \(gbUnit)", "Provisioned %",
